...
linux:
	...
	@rm -f $(LINUX_BUILD_DIR)/libfinkit_crypto_static_base.a
...
macos:
	...
	@echo "==> Cleaning intermediate build artifacts"
	@rm -f $(MACOS_BUILD_DIR)/libfinkit_crypto_static_base.a
...
bench:
	...
	@if [ -f $(LINUX_BUILD_DIR)/benchmarks/crypto_benchmarks ]; then \
		$(LINUX_BUILD_DIR)/benchmarks/crypto_benchmarks; \
	else \
...
format:
	...
	@if command -v clang-format >/dev/null 2>&1; then \
		find crypto-c include \( -name '*.c' -o -name '*.h' \) -exec clang-format -i {} \; ; \
	else \
...
clang-tidy:
	...
	@find crypto-c include \( -name '*.c' -o -name '*.h' \) \
		! -name 'platform_win.c' \
...
cppcheck:
	...
	@find crypto-c include \( -name '*.c' -o -name '*.h' \) -print | \
...
help:
	@echo "crypto Makefile - Build and Test Targets"
	@echo ""
	@echo "Build Targets:"
